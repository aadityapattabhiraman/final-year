...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, writer, num_examples=2):
    ...
    try:
        ...
        with os.open("stty size", "r") as console:
            _, console_width = consdole.read().split()
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            assert encoder_input.size(0) == 1, "Batch size must be 1 for vaalidation"
...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        # Mostly taken from https://huggingface.co/docs/tokenizers/quicktour
...
def train_model(config):
    ...
    model = get_model(config, tokenizer_src.get_vocab_size(), tokenizer_tgt.getvocab_size()).to(device)
    # Tensorboard
